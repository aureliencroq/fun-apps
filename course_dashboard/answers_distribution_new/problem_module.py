...
def fetch_all(course_key, store, problem_module_id=None):
    """
    ...
    if problem_module_id:
        qualifiers['qualifiers']['name'] = problem_module_id
    problem_modules = store.get_items(course_key, **qualifiers)
    return problem_modules

def get(course_key, problem_module_id):
...
def _ancestors_names(problem_module, store):
